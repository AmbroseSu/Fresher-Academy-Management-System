...
public interface UnitRepository extends JpaRepository<Unit, String> {
        ...
        @Query(value = "SELECT * FROM tbl_unit u " +
                        "WHERE (:name IS NULL OR  LOWER(u.name) LIKE LOWER(CONCAT('%', :name,'%'))) AND (:dayNumber IS NULL OR  LOWER(u.dayNumber) LIKE LOWER(CONCAT('%', :dayNumber,'%'))) AND u.status = TRUE "
                        +
        ...
        @Query(value = "SELECT COUNT(*) FROM tbl_unit u " +
                        ...
                        +
                        "AND (:duration IS NULL OR u.duration = :duration)", nativeQuery = true)
        Long countSearchSortFilter(@Param("name") String name, @Param("dayNumber") String dayNumber,
                        @Param("duration") Integer duration);

        @Query(value = "SELECT * FROM tbl_unit u " +
                        "WHERE (:name IS NULL OR  LOWER(u.name) LIKE LOWER(CONCAT('%', :name,'%'))) AND (:dayNumber IS NULL OR  LOWER(u.dayNumber) LIKE LOWER(CONCAT('%', :dayNumber,'%')))"
                        +
